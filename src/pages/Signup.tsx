    ...
    const { error } = await authClient.signUp.email(
      {
        ...
        name: formInfo.name,
        image: formInfo.imageUrl, // optional
        callbackURL: "/",
      },
      {
        onRequest: () => {
        ...
        onSuccess: (ctx) => {
  setLoading(false);
  toast.success(ctx.data?.message ?? "Signed up successfully!");
  navigate("/");
},
        onError: (ctx) => {
          setLoading(false);
    ...
    >
      ...
      >
        ...
        <Input
          ...
          className="border border-border bg-bg-light text-text-dark placeholder:text-text-muted focus:border-accent focus:ring-accent"
        />
        <Description className="text-text-muted">Profile picture URL (optional)</Description>
        <FieldError className="text-primary" />
      </TextField>
...
<div className="relative">
   <TextField
    ...
    <p className="text-center text-secondary text-sm my-2">Already Have Account? <span ><Link href="/login" className="text-primary">Login</Link></span></p>
    </div>
